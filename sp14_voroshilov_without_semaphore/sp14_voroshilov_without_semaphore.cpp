// sp14_voroshilov_without_semaphore.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
ClubState club;
volatile int countVisitorsWithPC = 0;
...
DWORD WINAPI ClientThread(LPVOID _client) {
    ...
    bool isTimeout = true;
    while (GetTickCount64() - client->arriveTick < 3000) {
        if (countVisitorsWithPC < CLUB_CAPACITY) {
            countVisitorsWithPC++;
